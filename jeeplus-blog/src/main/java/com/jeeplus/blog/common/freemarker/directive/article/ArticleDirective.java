...
public class ArticleDirective implements TemplateDirectiveModel {
    ...
    public void execute(Environment env, @SuppressWarnings("rawtypes") Map params, TemplateModel[] tm, TemplateDirectiveBody body) throws TemplateException, IOException {
        ...
        try {
            ...
            if (!tag.isEmpty()) {
                //通过标签名取文章
                p=articleService.getArticleByLable(params.get("tag").toString(),website_id, page,limit);
            }else if (!fulltext.isEmpty()) {
                //全文检索
                p=articleService.getFullTextSearch(page, limit, website_id, fulltext, "articleTitle","articleContent","id");
            }else{
                p=articleService.getArticleList(page,limit, website_id, map);
            }
            if (fulltext.isEmpty()) {
            	lis=(List<BlogArticle>) p.getData();
                String summary=null;
                for (BlogArticle article : lis) {
